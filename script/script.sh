...
eliminar_carpeta() {
  # Comprobar si el argumento es válido
  if [ -z "$1" ]; then
    echo "Error: falta el nombre de la carpeta"
    return 1
  fi

  # Comprobar si la carpeta existe y eliminarla con el comando rm -rf
...
cd ..
cd MoogleServer
eliminar_carpeta bin
eliminar_carpeta obj
cd ..
cd MoogleEngine 
eliminar_carpeta bin
eliminar_carpeta obj
...
case $1 in 
	...
	show_slides)
		...
		echo "Abriendo el pdf de la presentacion..."
